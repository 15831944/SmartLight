...
int Q_protocolIEC104Master::decodeMsg_U(BYTE *msgbuf, unsigned int len)
{
	...
	TRACE("--------------------Q_protocolIEC104Master::decodeMsg_U  usTemp_ServSendNum = %d------------------------\n",usTemp_ServSendNum);

	if (usTemp_ServSendNum & CMD_STARTDT_CON)//STARTDT  子站确认
	{
		TRACE("--------------------Q_protocolIEC104Master::decodeMsg_U -- //STARTDT  子站确认  bStartFlg=true ------------------------\n");
		//初使结束
		bStartFlg=true;
	}
	...
	else if (usTemp_ServSendNum & CMD_STARTDT_CON)//STARTDT  生效 从站发送确认
	{
		//在变量中增加一个，可以发送I针标识发送启动传输确认 可以发送数据
		m_bStopSend=false;
	}
	else if (usTemp_ServSendNum & CMD_STOPDT_CON)//STOPDT 生效
	{
		//停止发送数据
		m_bStopSend=true;
	}
...
int Q_protocolIEC104Master::decodeMsg_I(BYTE *msgbuf, unsigned int len)
{
	...
	case 30: //Single-point information with time tag CP56Time2a
		...
		break;
		//case 31://双点带时间格式的
		//	func_M_DP_TB_1(msgbuf,len,asdu,seq);
		//	break;
		//case 32://步位置信息
	...
	if (msg->seq)
	{
		...
		while (pos < msg->len)
		{
			...
			if(!m_pMgr->recordDataValue(&dataArray,1))
			{
				...
				{
					sprintf_s(LastError,sizeof(LastError),"Can't find right YC point in realDB,Please check yc.xml ,Channel=%d,device=%d,addr=%d",m_ProSetting->getChannelID(),msg->asdu,adr);
					//OnShowMsg(LastError,sizeof(LastError));
					//新规约打印
					SendMsgPrint(m_ProSetting->getChannelID(),eMsgTypeCh,(BYTE*)LastError,strlen(LastError)+1);
				}
...
void Q_protocolIEC104Master::func_M_DP_NA_1(Q_104ApduEx *msg)
{
	...
	*/
	...
	if (msg->seq)
	{
		...
		pos = sizeof(Q_IEC104_ASDU_header)+3;
		Q_DIQ104	*pi;
		while (pos < msg->len)
		{
...
{ 
	...
	else
	{
		...
		if (m_bErrMsg)
		{
		//OnErrMsg(LastError,sizeof(LastError));
		}
		*/
		//新规约打印
		SendMsgPrint(m_ProSetting->getChannelID(),eMsgTypeError,(BYTE*)LastError,strlen(LastError)+1);
