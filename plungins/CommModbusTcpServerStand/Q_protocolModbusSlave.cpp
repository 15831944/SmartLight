...
bool Q_protocolModbusSlaveSetting::getCloseSyncCode()
{
	bool flg=false;
...
bool Q_protocolModbusSlave::explainFrameYX(Q_MODBUS_ASDU *header,BYTE *data,const int &len)
{
	...
	if(m_PrintSetting->getCloseSyncCode())
	{
		asdu.header.codeHigh=header->header.codeHigh;
		asdu.header.codeLow=header->header.codeLow;
	}
	else
	{
...
*/
	...
	if(m_PrintSetting->getYcUpType()==YcUpTypeINT2)
	{
		...
		for(int i=0;i<regCount;i++)
		{
			...
			curs.fValue = dataArray.val.fValue;//乘过系数的数值
			//curs.factor = 1;

			BYTE tmp[2];
			memset(tmp,NULL,sizeof(tmp));

			short s_n=(short)(curs.fValue);//只有双字节整型读系数
