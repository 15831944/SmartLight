	...
	else
	{
		if(1==nRet)
		{
			...
			if(m_pSocInfo->checkCFStatus()==SOC_POWER_STATUS_Charge)
			{
				...
				if(!m_funcRtdb.equalDouble(m_pSocInfo->p_Unit->fValUp,-fChargetKw))
				{
					//同时此值
					pcsControlMethod(-fChargetKw-m_pSocInfo->p_Unit->fValUp);
				}
		...
		else
		{
			if(m_pSocInfo->checkCFStatus()==SOC_POWER_STATUS_Discharge)
			{
				...
				if(!m_funcRtdb.equalDouble(m_pSocInfo->p_Unit->fValUp,fDischargetKw))
				{
					//同时此值
					pcsControlMethod(fDischargetKw-m_pSocInfo->p_Unit->fValUp);
				}
...
int agcCalc::UpVoltage()
{
	...
	else if(m_pSocInfo->checkCFStatus()==SOC_POWER_STATUS_Discharge)
	{
		...
		else
		{
			...
			float fDischargeMaxPower = m_funcRtdb.getAgcAnalog(eAgcAnalogSocDischargePMax);//储能放电最大功率
			bool bOverLoad=false;
			if(m_pSocInfo->p_Unit->fValUp>0)
			{
				if(m_funcRtdb.equalDouble(m_pSocInfo->p_Unit->fValUp,fDischargeMaxPower)||
...
int agcCalc::DownVoltage()
{
	...
	if(m_pSocInfo->p_Unit->iStatus==SOC_PCS_RUN_STATUS_Run)
	{
		if(m_pSocInfo->checkCFStatus()==SOC_POWER_STATUS_Charge)
		{
			float fChargeMaxPower = m_funcRtdb.getAgcAnalog(eAgcAnalogSocChargePMax);//储能充电最大功率
...
int agcCalc::cheakCommonVarible()
{
	...
	else if(m_funcRtdb.equalDouble(m_pSocInfo->p_Unit->fValUp,0))
	{
...
float agcCalc::getNormalChargePower()
{
	...
	if(ret>0&& ret<m_pSocInfo->p_Unit->fcurCap && ret<fPhvSum&&ret>fChargetKw)
	{
		return ret;
	}
	else
	{
		...
		memset(m_LastError,0,sizeof(m_LastError));
		sprintf(m_LastError ,"充电功率计算逻辑错误，请检查，置充电功率为%.2fkW,计算预期结果%.2f,光伏功率%.2f,储能额定%.2f",fChargetKw,ret,fPhvSum,m_pSocInfo->p_Unit->fcurCap);
		writeBuffer(m_LastError);
		ret=fChargetKw;
	}
	return ret;
